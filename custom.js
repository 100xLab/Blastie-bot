function getCustomContractSource(
    chain,
    tokenName,
    tokenSymbol,
    totalSupply,
    ...
    burnFeeOnBuy = 0,
    burnFeeOnSell = 0,
    marketingWalletAddress = "0x000000000000000000000000000000000000dEaD",
    maxTransactionAmountBuyPercentage = 0,
    maxTransactionAmountSellPercentage = 0,
    maxWalletAmountPercentage = 0,
    website = "None",
    twitter = "None",
    telegram = "None",
    description = "None",
    ...
    // Default decimals for ERC20 token
    const decimals = 18;
    const uniqueContractName = `StandardToken${userId}`;

    // Convert total supply based on decimals
    const totalSupplyWithDecimals = totalSupply * Math.pow(10, decimals);

    console.log("totalSupplyWithDecimals", totalSupplyWithDecimals);

    const calculatePercentage = (percentage, total) => {
        if (percentage === 0) return BigInt(0);
        return BigInt(Math.round(total * (percentage / 100)));
    };

    const maxTransactionAmountBuy = calculatePercentage(maxTransactionAmountBuyPercentage, totalSupplyWithDecimals);
    const maxTransactionAmountSell = calculatePercentage(maxTransactionAmountSellPercentage, totalSupplyWithDecimals);
    const maxWalletAmount = calculatePercentage(maxWalletAmountPercentage, totalSupplyWithDecimals);

    // Convert BigInt values to strings
    const maxTransactionAmountBuyStr = maxTransactionAmountBuy.toString();
    const maxTransactionAmountSellStr = maxTransactionAmountSell.toString();
    const maxWalletAmountStr = maxWalletAmount.toString();


    // Template of the smart contract with placeholders
    ...
    contract ${uniqueContractName} is Context, IERC20, Ownable {
        ...
        uint256 private _tTotal = ${totalSupply} * (10 ** uint256(_decimals));
        uint256 private _tTotalSupply = ${totalSupply} * (10 ** uint256(_decimals));
        uint256 private _rTotal = (MAX - (MAX % _tTotal));
        uint256 private _tFeeTotal;

    
        uint public ReflectionFeeonBuy=${reflectionFeeOnBuy * 10};
        uint public ReflectionFeeonSell=${reflectionFeeOnSell * 10};
    
        uint public liquidityFeeonBuy=${liquidityFeeOnBuy * 10 };
        uint public liquidityFeeonSell=${liquidityFeeOnSell * 10};
    
        uint public marketingFeeonBuy=${marketingFeeOnBuy * 10};
        uint public marketingFeeonSell=${marketingFeeOnSell * 10};
    
        uint public burnFeeOnBuy=${burnFeeOnBuy * 10};
        uint public burnFeeOnSell=${burnFeeOnSell * 10};
        ...
        address public marketingWallet=${marketingWalletAddress};
    
        address public referralWallet;
        uint256 public serviceFee;
        uint256 public referralCommission;
    
        uint256 public maxTransactionAmountBuy=${maxTransactionAmountBuyStr};
        uint256 public maxTransactionAmountSell=${maxTransactionAmountSellStr};
    
        uint256 public maxWalletAmount=${maxWalletAmountStr};
        ...
        constructor() payable {
            //address router = 0x000000000000000000000000000000000000dEaD;
        //require(msg.value >= 0.05 ether, "Insufficient value for fee receiver");
        
        
        //serviceFee = 0.05 ether;
        //payable(0x000000000000000000000000000000000000dEaD).transfer(serviceFee);
        ...
        function configureYieldAndGas(YieldMode yieldMode, GasMode gasMode) external onlyOwner {
            blastContract.configure(yieldMode, gasMode, governor);
        }
    
        function claimYield(uint256 amount, address recipient) external onlyOwner {
            blastContract.claimYield(address(this), recipient, amount);
        }
    
        function claimAllYield(address recipient) external onlyOwner {
            blastContract.claimAllYield(address(this), recipient);
        }
    
        function claimGas(uint256 amount, address recipient) external onlyOwner {
            // Assuming the gas claim logic is similar to yield
            blastContract.claimGas(address(this), recipient, amount, 0); // Adjust as per gas claiming logic
        }
    
        function readMyYield() external view returns (uint256) {
            return blastContract.readClaimableYield(address(this));
        }
